...
# ╔═╡ 9c6a7954-b59d-11ef-28eb-29570739c322
begin
...
# ╔═╡ 9449894e-9139-409c-8977-51b2d7c8c380
using StatsBase

# ╔═╡ 3854e764-6282-4540-96a0-1a11e813d9d0
begin
d = CSV.read("/store/users/ehinger/student_projects/2024-2AFC-diagonal/scripts/tidy_data.csv",DataFrame)
d.condition .= String.(d.condition)
	d.fake .= 1
for g in groupby(d,[:subject,:condition])
	g.n_trials .= size(g,1)
end
	d = hcat(d,DataFrame(eachrow(hcat(split.(d.condition,'-')...)),["first","second"]))

	coords = parse.(Int,hcat(split.(replace.(d.coord,"("=>"",")"=>""," "=>""),',')...))

	d = hcat(d,DataFrame(eachrow(coords),["target_x","target_y"]))
	coord_to_name = Dict(
		(-160,-277)=>"7",
		(160, 277)=>"1",
    (-277, -160)=>"8",
    ...
    (0,320)=>"12",
	(0,-320)=>"6")
	d.choice = [coord_to_name[(x,y)] for (x,y) in eachrow(d[:,[:target_x,:target_y]])]
	d
end
...
begin
	#this  doesnt work because not all choices are even available for all subjects
#d_avg = @by(d,[:subject,:choice],:count= length(:reaction_time),:perc = (length(:reaction_time))/(:n_trials[1]),:m_rt = mean(:reaction_time))
	d_avg = DataFrame()
for subject = unique(d.subject)
	for choice = string.(1:12)
		d_loop = @subset(d,:choice.==choice,:subject.==subject)
		if size(d_loop,1) == 0
			m_rt = missing
			perc = 0
		else
			m_rt = mean(d_loop.reaction_time)
			perc = size(d_loop,1)/d_loop.n_trials[1]
		end
		global d_avg = vcat(d_avg,DataFrame(:choice=>choice,:subject=>subject, :reaction_time => m_rt,
				  :perc => perc))
	end
end
end
...
# ╔═╡ 8bc931fe-8dc1-47a6-9c07-af09c5c89c0f
	d_avg_avg = @by(d_avg,:choice,:mean_perc=mean(:perc),:se_perc=std(:perc)./sqrt(length(unique(d.subject))),:std_perc=std(:perc))

# ╔═╡ 0aee5d43-a6bf-4311-966f-4a98495b076d
begin

data(d_avg)*
	mapping(:choice,:perc)*
	visual(Beeswarm,algorithm=PseudorandomJitter(),alpha=0.2)+
data(d_avg_avg)*
	mapping(:choice,:mean_perc,:se_perc)*
	(
		visual(Errorbars)+
		visual(Scatter,color=:white,strokecolor=:black,strokewidth=1,markersize=10)
	)|>x->draw(x)
...
# ╔═╡ 2f35f170-0edd-4a3a-80a5-4169ff5c1da3
begin
data(d_avg)*mapping(:choice,:reaction_time)*visual(Scatter,alpha=0.3)+
data(@by(d_avg,:choice,:mean_rt=mean(winsor(collect(skipmissing(:reaction_time)),prop=0.2)),:se_rt=std(winsor(collect(skipmissing(:reaction_time)),prop=0.2))./sqrt(length(unique(d.subject)))))*
mapping(:choice,:mean_rt,:se_rt)*
(visual(Errorbars,color=:darkblue)+visual(Scatter,color=:white,strokecolor=:lightblue,strokewidth=2,markersize=15))|>x->draw(x)
ylims!(-0.1,2)
	current_figure()
end

# ╔═╡ dbc26846-eaeb-4a68-b1a4-5b58e06700b3
mean(winsor(collect(skipmissing([1,2,3,4,5,missing]))))

# ╔═╡ 208318e7-e373-48ca-b7f5-559d78eb85b8
@by(d_avg,:choice,:mean_rt=mean(skipmissing(:reaction_time)),:se_rt=std(skipmissing(:reaction_time))./sqrt(length(unique(d.subject))))

# ╔═╡ dc7ee746-63c1-4ab2-9196-f6a7e0b30bf6
d_avg

# ╔═╡ ccfe79d7-37b9-4cfe-9124-d16105368778
length(unique(d.subject))

# ╔═╡ b163ac5b-6f92-4408-aa94-e8fe080b63c2
data(d)*mapping(:condition,:choice)*visual(Violin)|>draw
